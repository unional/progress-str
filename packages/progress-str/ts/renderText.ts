...
function formatText(entry: ValueEntry) {
	switch (entry.textStyle) {
		...
		case 'number':
			return entry.value === undefined ? '---' : entry.value.toFixed(entry.actualDigits)
		case 'ratio': {
			const max =
				entry.max === undefined ? '---' : entry.digits !== undefined ? entry.max.toFixed(entry.actualDigits) : entry.max
			return entry.value === undefined ? `---/${max}` : `${entry.value.toFixed(entry.actualDigits)}/${max}`
		}
	}
}
