...
import { LengthTooShort, MissingMaxValue } from './errors.js'
import { InvalidMarker, MaxValueOutOfBound, progressBar } from './index.js'
...
test('for number style, when max is less than or equal to 1, value is rounded to 1/10 of the max value', () => {
	...
	assertRendering(
		progressBar({ value: { max: 0.54, textStyle: 'number' } }).render(0.2234),
		'[---------|------------] 0.223'
	)
})
...
test('for number style, when max is greater than 1, value is rounded to the whole number', () => {
	assertRendering(
		progressBar({ value: { max: 100, textStyle: 'number' } }).render(51.2),
		'[------------|-----------] 51 '
	)
})
...
test('for ratio style, when max is less than or equal to 1, value is rounded to 1/10 of the max value', () => {
	...
	assertRendering(
		progressBar({ value: { max: 0.54, textStyle: 'ratio' } }).render(0.2234),
		'[------|----------] 0.223/0.54'
	)
})
...
test('for ratio style, when max is greater than 1, value is rounded to the whole number', () => {
	assertRendering(
		progressBar({ value: { max: 100, textStyle: 'ratio' } }).render(51.2),
		'[----------|---------] 51/100 '
	)
})
...
test('digits of the text can be customized', () => {
	...
	assertRendering(progressBar({ value: { digits: 2 } }).render(0.515), '[----------|---------] 51.50% ')
	assertRendering(
...
test('markers can be colored', () => {
	...
	assertRendering(
		bar.render(0.5),
		`${leftBracketMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${completedMarker}${marker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${incompleteMarker}${rightBracketMarker} 50% `
	)
})
